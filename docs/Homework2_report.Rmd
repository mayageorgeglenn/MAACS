...
library(tidyverse)
library(dplyr)
dataset <- read.csv("/Users/mayaglenn/SDS375/homework2/MAACS/docs/maacs.csv.gz")
...
fev1_func <- function(dataset) {
  ...
  younger <- dataset$fev1[dataset$age <= median_age]
  older <- dataset$fev1[dataset$age > median_age]
  
  # Compare mean FEV1 values
  return(mean(older, na.rm = TRUE) > mean(younger, na.rm = TRUE))
}

```

The dataset was split into younger and older groups based on the median age, and by comparing the average FEV1 values of each group, it is `r fev1_func(dataset)` that older children have higher FEV1 values than younger children.
...
#premise one: significant p-value
unname(summary(model)$coefficients["age", "Pr(>|t|)"])
...
1. The p value is less than 2e-16, which is also less than an Alpha of 0.01 or 0.05. This indicates that the positive relationship between age and FEV1 is statistically significant.

2. The correlation between age and FEV1 is strong and positive, indicating that there is a direct positive association between age and FEV1. 

3. The plot illustrates a positive trend among the data points, where as age increases, so do the FEV1 levels. 
...
Function 1 checks to make sure that the p-value is less than 0.01 (and that the slope is positive), which confirms that premise one is `r summary_func(dataset)`.
